...
def do_file(args):
    ...
    try: 
        for fname in args[1:]:
            with open(fname, 'r') as input_file, open(strip_suffix(fname) + file_ext, 'w') as out_file,open(strip_suffix(fname) + '.lay', 'w') as lay_out:
                funct(input_file, out_file, lay_out)
